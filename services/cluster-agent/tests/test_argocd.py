...
def _secret(data: dict[str, str] | None) -> SimpleNamespace:
    if data is None:
        return None
    return SimpleNamespace(data={k: base64.b64encode(v.encode()).decode() for k, v in data.items()})


def _mock_secret_calls(existing_data: dict[str, str] | None):
...
@pytest.mark.asyncio
async def test_ensure_patched_writes_only_missing_or_stale_configmap_keys():
    desired = argocd.desired_notifications_data()
    desired_secret = argocd.desired_notifications_secret_data()
...
@pytest.mark.asyncio
async def test_ensure_patched_creates_configmap_when_absent():
    desired_secret = argocd.desired_notifications_secret_data()
    get_secret_patch, patch_secret_patch = _mock_secret_calls(desired_secret)
    with (
        get_secret_patch,
...
@pytest.mark.asyncio
async def test_ensure_patched_raises_when_notifications_secret_is_absent():
...
@pytest.mark.asyncio
async def test_ensure_patched_writes_secret_when_token_rotated():
    desired = argocd.desired_notifications_data()
    cm = SimpleNamespace(data=dict(desired))
...
def test_desired_notifications_data_references_secret_instead_of_embedding_token():
...
def test_webhook_body_escapes_all_dynamic_fields_with_tojson():
...
@pytest.mark.asyncio
async def test_discover_reports_patch_failed_not_found_on_genuine_patch_error():
    with (
...
class TestExtractVersion:
    ...
    def test_digest_pinned_with_no_tag_returns_none_not_the_digest(self):
        assert argocd._extract_version(
            "quay.io/argoproj/argocd@sha256:" + "a" * 64
        ) is None

    def test_registry_host_with_port_and_no_tag_returns_none(self):
        assert argocd._extract_version("registry.internal:5000/argoproj/argocd") is None

    def test_registry_host_with_port_and_tag(self):

...
- **External MCP connector** (`GET/POST/DELETE /mcp` on ingest, `app/routers/mcp_connector.py`): a transparent Streamable HTTP passthrough letting Claude Desktop, a claude.ai custom connector, or a ChatGPT connector reach the MCP server directly, the way a remote GitHub MCP server works. Authenticates the external caller with an org API key (`verify_api_key`, same as the REST API) and swaps it for `MCP_INTERNAL_TOKEN` + the resolved `X-Org-Id` before forwarding — the external key never reaches mcp-server, and mcp-server's own trust model is unchanged. No Caddy change needed; it's just another route on ingest, already the sole public origin.
- **Per-org Slack delivery (E23-T5)** lives entirely in the agent, not ingest — `agent/notifications.py::notify_deploy_alert`/`notify_deploy_recovered`, called from `run.py`/`reconciliation.py` right after `fire_alert`/`resolve_alert`. The agent decrypts each workspace's Fernet-encrypted bot token itself (`INTEGRATION_ENC_KEY`, shared with ingest) and posts to the Slack Web API directly — no ingest hop. Best-effort with one 429 retry; a channel hitting a fatal error (`channel_not_found`, `invalid_auth`, etc.) gets auto-disabled rather than retried forever. **Don't build a second delivery path through ingest for this** — that duplication was tried and reverted 2026-09-08 (see project memory) because it caused every alert to send two separate Slack messages.
...
GRAFANA_RENDERER_TOKEN=  # Shared secret between grafana and renderer containers; Grafana 13+ won't start rendering without it
GEMINI_API_KEY=          # chat proxy's Gemini API key (E11-T1, ai.google.dev free tier) — never exposed to the browser
...
- **Alertmanager's `webhook_configs` URL is topology-dependent** (`deploy/helm-values/kube-prometheus-stack.yaml` and `deploy/alertmanager/alertmanager.yml`) — `host.docker.internal:8000` only reaches ingest when Alertmanager (in the Kind cluster) and ingest are on the same docker host (pure local dev). For the Azure VM demo (Kind cluster local, ingest remote), it must be the VM's public URL instead, or every alert resolution silently fails to reconcile `alerts.resolved_at` via that path (the agent's own reconciliation loop still resolves it independently, so this only affects timestamp accuracy/redundancy, not whether alerts fire).

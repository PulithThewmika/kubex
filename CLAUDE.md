...
5. After merge: close the task issue if `Closes #N` didn't already auto-close it, move its board item to Done, sync local `dev`. When every sibling task under an epic is Done, close the epic issue and move its board item to Done too.
6. If implementing a task surfaces a real architecture gap or a spec that's internally inconsistent with what's already built (not just a design preference) — ask the user via a direct question rather than silently choosing a workaround; save legitimate but out-of-scope follow-up work as a flagged task (`spawn_task`) instead of expanding the current PR.

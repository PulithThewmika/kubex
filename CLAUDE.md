...
- **agent → ingest → Slack** (`POST /internal/notify`, `app/routers/notify_internal.py`): the detection agent has no Slack integration of its own — only ingest holds the OAuth bot tokens and each org's `notification_channels` (configured at `/settings`). The agent (`agent/notify.py`) POSTs an `{org_id, event_type, text, service_id}` envelope with bearer `MCP_INTERNAL_TOKEN` (same shared secret, agent→ingest direction this time); ingest resolves the org's connected workspace + matching channels and calls the Slack Web API directly. Best-effort both ways — a delivery failure never blocks the alert firing/resolving in Postgres.

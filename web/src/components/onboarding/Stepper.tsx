        ...
        return (
          <li key={label} className="flex flex-1 items-center last:flex-none">
            <div className="flex items-center gap-2">
              ...
              <span
                className={`hidden text-sm sm:inline ${active ? 'font-medium text-text' : 'text-text-muted'}`}
              >
                {label}
              </span>
            </div>
            {i < steps.length - 1 && (
              <span className={`mx-3 h-px flex-1 ${done ? 'bg-accent' : 'bg-border'}`} aria-hidden="true" />
            )}
          </li>
        )
      })}
